...
describe('Utils', () => {
  ...
  describe('colUserLabel()', () => {
    it('should return custom label when set', () => {
      const db = (globalThis as any).db;
      db.columnLabels = { Orders: { Amount: 'Total $' } };
  ...
  describe('setColLabel()', () => {
    it('should set a custom label', () => {
      ...
      expect(db.columnLabels['Orders']['Amount']).toBe('Revenue');
    });

    it('should clear label when set to physical column name', () => {
      const db = (globalThis as any).db;
      db.columnLabels = { Orders: { Amount: 'Revenue' } };
      setColLabel('Orders', 'Amount', 'Amount');
      expect(db.columnLabels['Orders']?.['Amount']).toBeUndefined();
    });
    ...
    it('should clean up empty table label object', () => {
      ...
      db.columnLabels = { Orders: { Amount: 'Revenue' } };
      setColLabel('Orders', 'Amount', 'Amount');
      expect(db.columnLabels['Orders']).toBeUndefined();
    });
  });

  describe('colDisplayLabel()', () => {
    it('should return formatted display label for physical column', () => {
      const db = (globalThis as any).db;
      db.columnLabels = { Orders: { Amount: 'Revenue' } };
      db.base = 'Orders';
      db.lookups = [];
      db.calcStages = [];
      const label = colDisplayLabel('Amount');
      expect(label).toContain('Orders');
      expect(label).toContain('Revenue');
      expect(label).toContain('\u2192');
    });

    it('should return alias when not found in map', () => {
      const db = (globalThis as any).db;
      db.base = 'Orders';
      db.lookups = [];
      db.calcStages = [];
      expect(colDisplayLabel('NonExistent')).toBe('NonExistent');
    });
  });

  describe('colExportLabel()', () => {
    it('should return user label for physical column', () => {
      const db = (globalThis as any).db;
      db.columnLabels = { Orders: { Amount: 'Revenue' } };
      db.base = 'Orders';
      db.lookups = [];
      db.calcStages = [];
      expect(colExportLabel('Amount')).toBe('Revenue');
    });
    ...
    it('should return physical column name when no label', () => {
      ...
      db.lookups = [];
      db.calcStages = [];
      expect(colExportLabel('Amount')).toBe('Amount');
    });

    it('should return alias when not found', () => {
      const db = (globalThis as any).db;
      db.base = 'Orders';
      db.lookups = [];
      db.calcStages = [];
      expect(colExportLabel('NonExistent')).toBe('NonExistent');
    });
  });

  describe('buildExportHeaderMap()', () => {
    it('should build header map for columns', () => {
      const db = (globalThis as any).db;
      db.columnLabels = {};
      db.base = 'Orders';
      db.lookups = [];
      db.calcStages = [];
      const map = buildExportHeaderMap(['OrderId', 'Amount']);
      expect(map['OrderId']).toBe('OrderId');
    ...
    it('should deduplicate headers', () => {
      ...
      const customMap = new Map([
        ['col1', { tid: 'Orders', col: 'Amount' }],
        ['col2', { tid: 'Orders', col: 'Amount' }],
      ]);
      const map = buildExportHeaderMap(['col1', 'col2'], customMap as any);
      expect(map['col1']).toBe('Amount');
      expect(map['col2']).toBe('Amount 2');
    });
    ...
    it('should handle triple duplicates', () => {
      ...
      db.calcStages = [];
      const customMap = new Map([
        ['a', { tid: 'Orders', col: 'Amount' }],
        ['b', { tid: 'Orders', col: 'Amount' }],
        ['c', { tid: 'Orders', col: 'Amount' }],
      ]);
      const map = buildExportHeaderMap(['a', 'b', 'c'], customMap as any);
      expect(map['a']).toBe('Amount');
      expect(map['b']).toBe('Amount 2');
      expect(map['c']).toBe('Amount 3');
    });
  });
