...
export function hydrateState(payload: Record<string, any>): { next: Record<string, unknown>; brokenRefs: string[]; nextExcludedRows: Record<string, Set<number>> } {
  const next: Record<string, any> = {};
  const brokenRefs = [];

  const nextAvailableCols = () => {
    const cols   = projectedColsUpToLookup((next.lookups || []).length, next as any);
  ...
  for (const lk of (payload.lookups || [])) {
    ...
    const leftAvail = baseLoaded ? projectedColsUpToLookup(next.lookups.length, next as any) : [];
