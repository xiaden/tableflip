export function renderWhereClause(colRef: string, op: string, val: string, params: unknown[], opts?: { numericHint?: boolean }): string | null {
  const likeEsc      = (v: string): string => v.replace(/%/g, '\\%').replace(/_/g, '\\_');
