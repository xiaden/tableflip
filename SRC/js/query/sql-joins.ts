...
export function renderFromJoinWhere(plan: QueryPlan): JoinClauseResult {
  ...
  function ref(alias: string): string {
  ...
  if (hasStacks) {
    ...
    const unionParts = allTids
      .filter((tid: string) => src.tablesById && src.tablesById.has(tid))
      .map((tid: string) => {
        const tCols  = src.tablesById.get(tid)!.cols;
        const selStr = [`"_rowno"`, ...baseCols
