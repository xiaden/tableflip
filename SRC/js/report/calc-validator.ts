...
function validateDateMode(_ctx: CalcValidatorCtx): string | null {
  return null;
}
...
export function checkCalcError(calc: CalcStage, i: number): string | null {
  ...
  const duplicates = (db.calcStages || []).filter((c: CalcStage, idx: number) => idx !== i && (c.alias || '').trim() === alias);
