...
export function renderResults(result: Record<string, unknown>): void {
  ...
  if (db.colState) {
    gridResult.applyColumnState(db.colState as unknown as Array<Record<string, unknown>>);
  }

  renderMergeToggles(cols);
}
if (typeof window !== 'undefined') (window as unknown as Record<string, unknown>).renderResults = renderResults;

function _saveResultColState(): void {
  if (gridResult) db.colState = gridResult.getColumnState() as unknown as Record<string, unknown> | null;
}
...
export function renderPreviewDropdown(): void {
  ...
  sel.innerHTML = '<option value="">— select a table to preview —</option>' +
    ids.map((id: string) => `<option value="${id}">${h(db.tables[id].name)}</option>`).join('');
...
export function loadPreview(): void {
  ...
  try {
    rows = execQuery(`SELECT "_rowno", ${t.cols.map((c: string) => quoteId(c)).join(', ')} FROM ${quoteId(id)} LIMIT ${cap}`);
  } catch (ex) {
    meta.textContent = 'Error loading preview';
    wrap.innerHTML   = '<div class="empty"><div class="empty-icon">❌</div><div>' + h((ex as Error).message) + '</div></div>';
    return;
  }
  ...
  const excludeColDef: Record<string, unknown> = {
    ...
    cellRenderer: (params: { value: number; data: Record<string, unknown> }) => {
      ...
      const preview = t.cols
        .filter((c: string) => c !== '_rowno')
        .map((c: string) => rowData[c] == null ? '' : String(rowData[c]))
        .filter((v: string) => v !== '')
        .slice(0, 6)
        .join(' · ');
...
if (typeof window !== 'undefined') (window as unknown as Record<string, unknown>).loadPreview = loadPreview;

function toggleRowExclusion(tableId: string, rowno: number): void {
...
export function clearExclusions(tableId: string): void {
  db.excludedRows[tableId] = new Set();
  renderQueryBuilder();
  loadPreview();
}
if (typeof window !== 'undefined') (window as unknown as Record<string, unknown>).clearExclusions = clearExclusions;

function makeResultCols(cols: string[]): Record<string, unknown>[] {
  const colMap = buildColSourceMap();
  const dataCols = cols.filter((c: string) => c !== '_rowno' && c !== '_row_type' && c !== '_isTotalsRow');
  return dataCols.map((c: string) => {
    const src       = colMap.get(c);
    const dispLabel = colDisplayLabel(c, colMap);
    const srcPhys = src as { tid: string; col: string } | undefined;
    const renamed   = (src && src.kind !== 'calc') ? db.columnLabels?.[srcPhys!.tid]?.[srcPhys!.col] : undefined;
...
function makePreviewCols(tid: string, physCols: string[]): Record<string, unknown>[] {
  const color = getTableColor(tid);
  return physCols.filter((c: string) => c !== '_rowno').map((c: string) => {
...
function _makeHeaderComponent(label: string, color: string | null, renamed: string | undefined, origCol: string | null, onRename: (() => void) | null, onClear: (() => void) | null): unknown {
  return class {
